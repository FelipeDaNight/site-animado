...
import { Activity, BookMarked, Info, Link2, Salad } from "lucide-react";
import Link from "next/link";
import { REGIOES_DIGESTORIO, type RegiaoDigestorio } from "@/data/types";
import { orgaosDigestivos } from "@/data/digestiveSystem";
import { DetailSection, BulletList } from "@/components/ui/detail-section";
import { Callout } from "@/components/ui/callout";

const DigestiveCanvas = dynamic(
  () => import("@/components/anatomy/digestive-canvas").then((mod) => mod.DigestiveCanvas),
  {
    ssr: false,
    loading: () => (
...
type RegiaoOuTodos = RegiaoDigestorio | typeof CORPO_TODO;

export function DigestiveExplorer() {
  const [regiao, setRegiao] = useState<RegiaoOuTodos>(CORPO_TODO);
  const [selectedSlug, setSelectedSlug] = useState<string | null>(null);

  const orgaosDaRegiao = useMemo(
    () => (regiao === CORPO_TODO ? orgaosDigestivos : orgaosDigestivos.filter((o) => o.regiao === regiao)),
    [regiao]
  );
  const regionMeshNames = useMemo(
    () => new Set(orgaosDaRegiao.flatMap((o) => o.meshNames)),
    [orgaosDaRegiao]
  );
  const selected = orgaosDigestivos.find((o) => o.slug === selectedSlug) ?? null;
  ...
  function handleCanvasSelect(meshName: string) {
    const entry = orgaosDigestivos.find((o) => o.meshNames.includes(meshName));
    if (entry) {
  ...
  return (
    <div>
      <div className="flex flex-wrap gap-1.5">
        {([CORPO_TODO, ...REGIOES_DIGESTORIO] as const).map((r) => (
          <button
            key={r}
            type="button"
        ...
        <div className="flex h-[60vh] flex-col rounded-2xl border border-border bg-background-raised lg:h-[68vh]">
          <div className="border-b border-border px-4 py-3">
            <h2 className="flex items-center gap-2 text-sm font-semibold text-foreground">
              <Salad className="h-4 w-4 text-brand" />
              {regiao === CORPO_TODO ? "Todas as estruturas" : regiao}
            </h2>
            <p className="mt-0.5 text-xs text-foreground-subtle">{orgaosDaRegiao.length} estruturas</p>
          </div>
          <div className="flex-1 overflow-y-auto p-2">
            {orgaosDaRegiao.map((o) => (
              <button
                key={o.slug}
                type="button"
