...
import { CIRCULATORY_MODEL_URL } from "@/data/circulatorySystem";

useGLTF.setDecoderPath("/draco/");
useGLTF.preload(SKELETON_MODEL_URL);
useGLTF.preload(CIRCULATORY_MODEL_URL);
...
// fully opaque), so the student can tell which body region a vessel is
...
function Model({ url, visibleMeshNames, selectedMeshNames, clickable, onSelect, onLoaded }: ModelProps) {
  ...
  // doesn't belong to visibleMeshNames (e.g. an occluding vessel or the
...
interface FrameCameraProps {
  circulatoryScene: THREE.Group | null;
  focusNames: Set<string> | null;
  padding: number;
  controlsRef: React.RefObject<OrbitControlsImpl | null>;
}

// Frames the camera on the circulatory model alone — the skeleton is only
// ever a dimmed backdrop and shouldn't influence the zoom, or a selected
// vessel with a short trajectory (e.g. a coronary artery) would always be
// framed as if it spanned the whole body. Three.js's camera is an
...
function FrameCamera({ circulatoryScene, focusNames, padding, controlsRef }: FrameCameraProps) {
  const camera = useThree((state) => state.camera);
  const invalidate = useThree((state) => state.invalidate);

  useEffect(() => {
    if (!circulatoryScene) return;
    const box = new THREE.Box3();
    const found = expandBoxByFocus(circulatoryScene, focusNames, box);
    if (!found) return;
    ...
    invalidate();
  }, [circulatoryScene, focusNames, padding, camera, controlsRef, invalidate]);
...
interface CirculatoryCanvasProps {
  visibleMeshNames: Set<string> | null;
  selectedMeshNames: Set<string> | null;
  onSelect: (meshName: string) => void;
}

export function CirculatoryCanvas({ visibleMeshNames, selectedMeshNames, onSelect }: CirculatoryCanvasProps) {
  const controlsRef = useRef<OrbitControlsImpl | null>(null);
  const [circulatoryScene, setCirculatoryScene] = useState<THREE.Group | null>(null);
  ...
  return (
    ...
    >
      ...
      <Suspense fallback={<Loading />}>
        <Model
          url={CIRCULATORY_MODEL_URL}
          visibleMeshNames={visibleMeshNames}
          selectedMeshNames={selectedMeshNames}
          clickable
          onSelect={onSelect}
          onLoaded={setCirculatoryScene}
        />
      </Suspense>
      <FrameCamera
        circulatoryScene={circulatoryScene}
