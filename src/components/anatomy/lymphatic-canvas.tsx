...
import { LYMPHATIC_MODEL_URL } from "@/data/lymphaticSystem";

useGLTF.setDecoderPath("/draco/");
useGLTF.preload(SKELETON_MODEL_URL);
useGLTF.preload(LYMPHATIC_MODEL_URL);
...
interface FrameCameraProps {
  lymphaticScene: THREE.Group | null;
  focusNames: Set<string> | null;
  padding: number;
  controlsRef: React.RefObject<OrbitControlsImpl | null>;
}

// Frames the camera on the lymphatic model alone — the skeleton is only ever
// a dimmed backdrop and shouldn't influence the zoom, or a selected organ
// with a small footprint (e.g. a single lymph node) would always be framed as if
// it spanned the whole torso. Three.js's camera is an imperative scene-graph
...
function FrameCamera({ lymphaticScene, focusNames, padding, controlsRef }: FrameCameraProps) {
  const camera = useThree((state) => state.camera);
  const invalidate = useThree((state) => state.invalidate);

  useEffect(() => {
    if (!lymphaticScene) return;
    const box = new THREE.Box3();
    const found = expandBoxByFocus(lymphaticScene, focusNames, box);
    if (!found) return;
    ...
    invalidate();
  }, [lymphaticScene, focusNames, padding, camera, controlsRef, invalidate]);
...
interface LymphaticCanvasProps {
  visibleMeshNames: Set<string> | null;
  selectedMeshNames: Set<string> | null;
  onSelect: (meshName: string) => void;
}

export function LymphaticCanvas({ visibleMeshNames, selectedMeshNames, onSelect }: LymphaticCanvasProps) {
  const controlsRef = useRef<OrbitControlsImpl | null>(null);
  const [lymphaticScene, setLymphaticScene] = useState<THREE.Group | null>(null);
  ...
  return (
    ...
    >
      ...
      <Suspense fallback={<Loading />}>
        <Model
          url={LYMPHATIC_MODEL_URL}
          visibleMeshNames={visibleMeshNames}
          selectedMeshNames={selectedMeshNames}
          clickable
          onSelect={onSelect}
          onLoaded={setLymphaticScene}
        />
      </Suspense>
      <FrameCamera
        lymphaticScene={lymphaticScene}
