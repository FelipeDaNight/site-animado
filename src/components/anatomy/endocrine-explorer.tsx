...
import { Activity, BookMarked, Info, Link2, FlaskConical } from "lucide-react";
import Link from "next/link";
import { REGIOES_ENDOCRINAS, type RegiaoEndocrina } from "@/data/types";
import { sistemaEndocrino } from "@/data/endocrineSystem";
import { DetailSection, BulletList } from "@/components/ui/detail-section";
import { Callout } from "@/components/ui/callout";

const EndocrineCanvas = dynamic(
  () => import("@/components/anatomy/endocrine-canvas").then((mod) => mod.EndocrineCanvas),
  {
    ssr: false,
    loading: () => (
...
type RegiaoOuTodos = RegiaoEndocrina | typeof CORPO_TODO;

export function EndocrineExplorer() {
  const [regiao, setRegiao] = useState<RegiaoOuTodos>(CORPO_TODO);
  const [selectedSlug, setSelectedSlug] = useState<string | null>(null);

  const estruturasDaRegiao = useMemo(
    () => (regiao === CORPO_TODO ? sistemaEndocrino : sistemaEndocrino.filter((o) => o.regiao === regiao)),
    [regiao]
  );
  ...
  const selected = sistemaEndocrino.find((o) => o.slug === selectedSlug) ?? null;
  ...
  function handleCanvasSelect(meshName: string) {
    const entry = sistemaEndocrino.find((o) => o.meshNames.includes(meshName));
    if (entry) {
  ...
  return (
    <div>
      <div className="flex flex-wrap gap-1.5">
        {([CORPO_TODO, ...REGIOES_ENDOCRINAS] as const).map((r) => (
          <button
            key={r}
            type="button"
      ...
      <div className="mt-6">
        ...
        {selected && (
          <article className="rounded-2xl border border-border bg-background-raised p-6">
            ...
            <DetailSection title="Hormônios secretados" icon={Activity}>
              <BulletList items={selected.hormoniosSecretados} />
            </DetailSection>
