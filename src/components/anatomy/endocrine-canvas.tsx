...
import { ENDOCRINE_MODEL_URL } from "@/data/endocrineSystem";

useGLTF.setDecoderPath("/draco/");
useGLTF.preload(SKELETON_MODEL_URL);
useGLTF.preload(ENDOCRINE_MODEL_URL);
...
interface FrameCameraProps {
  endocrineScene: THREE.Group | null;
  focusNames: Set<string> | null;
  padding: number;
  controlsRef: React.RefObject<OrbitControlsImpl | null>;
}

// Frames the camera on the endocrine model alone — the skeleton is only ever
// a dimmed backdrop and shouldn't influence the zoom, or a selected organ
// with a small footprint (e.g. the pineal gland) would always be framed as if
...
function FrameCamera({ endocrineScene, focusNames, padding, controlsRef }: FrameCameraProps) {
  const camera = useThree((state) => state.camera);
  const invalidate = useThree((state) => state.invalidate);

  useEffect(() => {
    if (!endocrineScene) return;
    const box = new THREE.Box3();
    const found = expandBoxByFocus(endocrineScene, focusNames, box);
    if (!found) return;
    ...
    invalidate();
  }, [endocrineScene, focusNames, padding, camera, controlsRef, invalidate]);
...
interface EndocrineCanvasProps {
  visibleMeshNames: Set<string> | null;
  selectedMeshNames: Set<string> | null;
  onSelect: (meshName: string) => void;
}

export function EndocrineCanvas({ visibleMeshNames, selectedMeshNames, onSelect }: EndocrineCanvasProps) {
  const controlsRef = useRef<OrbitControlsImpl | null>(null);
  const [endocrineScene, setEndocrineScene] = useState<THREE.Group | null>(null);
  ...
  return (
    ...
    >
      ...
      <Suspense fallback={<Loading />}>
        <Model
          url={ENDOCRINE_MODEL_URL}
          visibleMeshNames={visibleMeshNames}
          selectedMeshNames={selectedMeshNames}
          clickable
          onSelect={onSelect}
          onLoaded={setEndocrineScene}
        />
      </Suspense>
      <FrameCamera
        endocrineScene={endocrineScene}
