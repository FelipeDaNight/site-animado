...
import { BookMarked, Heart, Info, Link2, Route } from "lucide-react";
import Link from "next/link";
import { REGIOES_CIRCULATORIAS, type RegiaoCirculatoria } from "@/data/types";
import { sistemaCirculatorio } from "@/data/circulatorySystem";
import { DetailSection, BulletList } from "@/components/ui/detail-section";
import { Callout } from "@/components/ui/callout";

const CirculatoryCanvas = dynamic(
  () => import("@/components/anatomy/circulatory-canvas").then((mod) => mod.CirculatoryCanvas),
  {
    ssr: false,
    loading: () => (
...
type RegiaoOuTodos = RegiaoCirculatoria | typeof CORPO_TODO;

export function CirculatoryExplorer() {
  const [regiao, setRegiao] = useState<RegiaoOuTodos>(CORPO_TODO);
  const [selectedSlug, setSelectedSlug] = useState<string | null>(null);

  const estruturasDaRegiao = useMemo(
    () => (regiao === CORPO_TODO ? sistemaCirculatorio : sistemaCirculatorio.filter((e) => e.regiao === regiao)),
    [regiao]
  );
  const regionMeshNames = useMemo(
    () => new Set(estruturasDaRegiao.flatMap((e) => e.meshNames)),
    [estruturasDaRegiao]
  );
  const selected = sistemaCirculatorio.find((e) => e.slug === selectedSlug) ?? null;
  ...
  function handleCanvasSelect(meshName: string) {
    const entry = sistemaCirculatorio.find((e) => e.meshNames.includes(meshName));
    if (entry) {
  ...
  return (
    <div>
      <div className="flex flex-wrap gap-1.5">
        {([CORPO_TODO, ...REGIOES_CIRCULATORIAS] as const).map((r) => (
          <button
            key={r}
            type="button"
        ...
        <div className="flex h-[60vh] flex-col rounded-2xl border border-border bg-background-raised lg:h-[68vh]">
          ...
          <div className="flex-1 overflow-y-auto p-2">
            {estruturasDaRegiao.map((e) => (
              ...
              >
                <span>{e.nomeComum}</span>
                <span className="shrink-0 text-xs text-foreground-subtle">{e.tipo}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="mt-6">
        {!selected && (
          <Callout variant="info">
            ...
            ao esqueleto (esmaecido, como referência espacial) para localizar cada vaso ou câmara no corpo.
          </Callout>
        )}
        ...
        {selected && (
          <article className="rounded-2xl border border-border bg-background-raised p-6">
            ...
            <DetailSection title="Trajeto" icon={Route}>
              <p className="text-foreground-muted">{selected.trajeto}</p>
            </DetailSection>

            <DetailSection title="Ramos / tributárias" icon={Info}>
              <BulletList items={selected.ramosOuTributarias} />
            </DetailSection>
