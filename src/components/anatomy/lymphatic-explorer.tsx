...
import { Activity, BookMarked, Info, Link2, Waypoints } from "lucide-react";
import Link from "next/link";
import { REGIOES_LINFATICAS, type RegiaoLinfatica } from "@/data/types";
import { sistemaLinfatico } from "@/data/lymphaticSystem";
import { DetailSection, BulletList } from "@/components/ui/detail-section";
import { Callout } from "@/components/ui/callout";

const LymphaticCanvas = dynamic(
  () => import("@/components/anatomy/lymphatic-canvas").then((mod) => mod.LymphaticCanvas),
  {
    ssr: false,
    loading: () => (
...
type RegiaoOuTodos = RegiaoLinfatica | typeof CORPO_TODO;

export function LymphaticExplorer() {
  const [regiao, setRegiao] = useState<RegiaoOuTodos>(CORPO_TODO);
  const [selectedSlug, setSelectedSlug] = useState<string | null>(null);

  const estruturasDaRegiao = useMemo(
    () => (regiao === CORPO_TODO ? sistemaLinfatico : sistemaLinfatico.filter((o) => o.regiao === regiao)),
    [regiao]
  );
  ...
  const selected = sistemaLinfatico.find((o) => o.slug === selectedSlug) ?? null;
  ...
  function handleCanvasSelect(meshName: string) {
    const entry = sistemaLinfatico.find((o) => o.meshNames.includes(meshName));
    if (entry) {
  ...
  return (
    <div>
      <div className="flex flex-wrap gap-1.5">
        {([CORPO_TODO, ...REGIOES_LINFATICAS] as const).map((r) => (
          <button
            key={r}
            type="button"
      ...
      <div className="mt-6">
        {!selected && (
          <Callout variant="info">
            ...
            ao esqueleto (esmaecido, como referência espacial) para localizar cada linfonodo ou órgão no corpo.
          </Callout>
        )}
        ...
        {selected && (
          <article className="rounded-2xl border border-border bg-background-raised p-6">
            ...
            <DetailSection title="Drenagem" icon={Link2}>
              <p className="text-foreground-muted">{selected.drenagem}</p>
            </DetailSection>
