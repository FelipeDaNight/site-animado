...
import { Activity, BookMarked, Info, Link2, Brain } from "lucide-react";
import Link from "next/link";
import { REGIOES_SNC, type RegiaoSNC } from "@/data/types";
import { sistemaNervosoCentral } from "@/data/cnsSystem";
import { DetailSection, BulletList } from "@/components/ui/detail-section";
import { Callout } from "@/components/ui/callout";

const CnsCanvas = dynamic(
  () => import("@/components/anatomy/cns-canvas").then((mod) => mod.CnsCanvas),
  {
    ssr: false,
    loading: () => (
...
type RegiaoOuTodos = RegiaoSNC | typeof CORPO_TODO;

export function CnsExplorer() {
  const [regiao, setRegiao] = useState<RegiaoOuTodos>(CORPO_TODO);
  const [selectedSlug, setSelectedSlug] = useState<string | null>(null);

  const estruturasDaRegiao = useMemo(
    () => (regiao === CORPO_TODO ? sistemaNervosoCentral : sistemaNervosoCentral.filter((o) => o.regiao === regiao)),
    [regiao]
  );
  ...
  const selected = sistemaNervosoCentral.find((o) => o.slug === selectedSlug) ?? null;
  ...
  function handleCanvasSelect(meshName: string) {
    const entry = sistemaNervosoCentral.find((o) => o.meshNames.includes(meshName));
    if (entry) {
  ...
  return (
    <div>
      <div className="flex flex-wrap gap-1.5">
        {([CORPO_TODO, ...REGIOES_SNC] as const).map((r) => (
          <button
            key={r}
            type="button"
