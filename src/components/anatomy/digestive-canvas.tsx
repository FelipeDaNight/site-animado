...
import { DIGESTIVE_MODEL_URL } from "@/data/digestiveSystem";

useGLTF.setDecoderPath("/draco/");
useGLTF.preload(SKELETON_MODEL_URL);
useGLTF.preload(DIGESTIVE_MODEL_URL);
...
interface FrameCameraProps {
  digestiveScene: THREE.Group | null;
  focusNames: Set<string> | null;
  padding: number;
  controlsRef: React.RefObject<OrbitControlsImpl | null>;
}

// Frames the camera on the digestive model alone — the skeleton is only ever
// a dimmed backdrop and shouldn't influence the zoom, or a selected organ
// with a small footprint (e.g. the gallbladder) would always be framed as if
...
function FrameCamera({ digestiveScene, focusNames, padding, controlsRef }: FrameCameraProps) {
  const camera = useThree((state) => state.camera);
  const invalidate = useThree((state) => state.invalidate);

  useEffect(() => {
    if (!digestiveScene) return;
    const box = new THREE.Box3();
    const found = expandBoxByFocus(digestiveScene, focusNames, box);
    if (!found) return;
    ...
    invalidate();
  }, [digestiveScene, focusNames, padding, camera, controlsRef, invalidate]);
...
export function DigestiveCanvas({ visibleMeshNames, selectedMeshNames, onSelect }: DigestiveCanvasProps) {
  const controlsRef = useRef<OrbitControlsImpl | null>(null);
  const [digestiveScene, setDigestiveScene] = useState<THREE.Group | null>(null);
  ...
  return (
    ...
    >
      ...
      <Suspense fallback={<Loading />}>
        <Model
          url={DIGESTIVE_MODEL_URL}
          visibleMeshNames={visibleMeshNames}
          selectedMeshNames={selectedMeshNames}
          clickable
          onSelect={onSelect}
          onLoaded={setDigestiveScene}
        />
      </Suspense>
      <FrameCamera
        digestiveScene={digestiveScene}
