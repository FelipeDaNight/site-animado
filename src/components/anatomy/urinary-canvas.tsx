...
import { URINARY_MODEL_URL } from "@/data/urinarySystem";

useGLTF.setDecoderPath("/draco/");
useGLTF.preload(SKELETON_MODEL_URL);
useGLTF.preload(URINARY_MODEL_URL);
...
interface FrameCameraProps {
  urinaryScene: THREE.Group | null;
  focusNames: Set<string> | null;
  padding: number;
  controlsRef: React.RefObject<OrbitControlsImpl | null>;
}

// Frames the camera on the urinary model alone — the skeleton is only ever
// a dimmed backdrop and shouldn't influence the zoom, or a selected organ
// with a small footprint (e.g. a single ureter) would always be framed as if
...
function FrameCamera({ urinaryScene, focusNames, padding, controlsRef }: FrameCameraProps) {
  const camera = useThree((state) => state.camera);
  const invalidate = useThree((state) => state.invalidate);

  useEffect(() => {
    if (!urinaryScene) return;
    const box = new THREE.Box3();
    const found = expandBoxByFocus(urinaryScene, focusNames, box);
    if (!found) return;
    ...
    invalidate();
  }, [urinaryScene, focusNames, padding, camera, controlsRef, invalidate]);
...
interface UrinaryCanvasProps {
  visibleMeshNames: Set<string> | null;
  selectedMeshNames: Set<string> | null;
  onSelect: (meshName: string) => void;
}

export function UrinaryCanvas({ visibleMeshNames, selectedMeshNames, onSelect }: UrinaryCanvasProps) {
  const controlsRef = useRef<OrbitControlsImpl | null>(null);
  const [urinaryScene, setUrinaryScene] = useState<THREE.Group | null>(null);
  ...
  return (
    ...
    >
      ...
      <Suspense fallback={<Loading />}>
        <Model
          url={URINARY_MODEL_URL}
          visibleMeshNames={visibleMeshNames}
          selectedMeshNames={selectedMeshNames}
          clickable
          onSelect={onSelect}
          onLoaded={setUrinaryScene}
        />
      </Suspense>
      <FrameCamera
        urinaryScene={urinaryScene}
